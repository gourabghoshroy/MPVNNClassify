...
np.random.seed(111100)
tf.random.set_seed(11110000)
...
for i in range(20):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        model = Sequential()
        callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss',mode='min',patience=20,verbose=0)
        model.add(Dense(numnodes, input_dim=numnodes, activation='tanh'))                                  
